...
export function useFleetData() {
  ...
  const addAircraft = useCallback(async (immat, type, opts = {}) => {
    ...
    if (opts.blockBlock !== undefined) row_data.block_block = opts.blockBlock;
    ...
    const ac = { id: row.id, immat: row.immat, type: row.type, carbuType: row.carbu_type || "100LL", huileType: row.huile_type || "W100", blockBlock: !!row.block_block, activeFrom: row.active_from || null, activeTo: row.active_to || null };
  ...
  const updateAircraft = useCallback(async (id, fields) => {
    ...
    if (fields.blockBlock !== undefined) dbFields.block_block = fields.blockBlock;

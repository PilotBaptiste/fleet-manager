...
function dbToMonthly(rows) {
  ...
  rows.forEach(r => {
    m[`${r.ac_id}|${r.year}|${r.month}`] = {
      ...
      volsDec1pax: Number(r.vols_dec_1pax) || 0,
      volsDec2pax: Number(r.vols_dec_2pax) || 0,
      volsDec3pax: Number(r.vols_dec_3pax) || 0,
      volsInitiation: Number(r.vols_initiation) || 0,
      volsBia: Number(r.vols_bia) || 0,
      revenuBia: Number(r.revenu_bia) || 0,
      heuresVoltigeDec: Number(r.heures_voltige_dec) || 0,
      volsVoltigeDec: Number(r.vols_voltige_dec) || 0,
      heuresVintage: Number(r.heures_vintage) || 0,
      volsVintage: Number(r.vols_vintage) || 0,
    };
  });
  return m;
}
...
export function useFleetData() {
  ...
  const setMonthly = useCallback(async (acId, year, month, fields) => {
    ...
    const merged = {
      ...
      volsDec1pax: mergeF("volsDec1pax"),
      volsDec2pax: mergeF("volsDec2pax"),
      volsDec3pax: mergeF("volsDec3pax"),
      volsInitiation: mergeF("volsInitiation"),
      volsBia: mergeF("volsBia"),
      revenuBia: mergeF("revenuBia"),
      heuresVoltigeDec: mergeF("heuresVoltigeDec"),
      volsVoltigeDec: mergeF("volsVoltigeDec"),
      heuresVintage: mergeF("heuresVintage"),
      volsVintage: mergeF("volsVintage"),
    };
    const dbRow = {
      ...
      vols_dec_1pax: merged.volsDec1pax,
      vols_dec_2pax: merged.volsDec2pax,
      vols_dec_3pax: merged.volsDec3pax,
      vols_initiation: merged.volsInitiation,
      vols_bia: merged.volsBia,
      revenu_bia: merged.revenuBia,
      heures_voltige_dec: merged.heuresVoltigeDec,
      vols_voltige_dec: merged.volsVoltigeDec,
      heures_vintage: merged.heuresVintage,
      vols_vintage: merged.volsVintage,
    };

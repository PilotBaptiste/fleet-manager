...
function dbToAircraft(rows) {
  return rows.map(r => ({ id: r.id, immat: r.immat, type: r.type }));
}

function dbToRates(rows) {
  return rows.map(r => ({
    id: r.id, acId: r.ac_id, field: r.field, value: Number(r.value),
    fromYear: r.from_year, fromMonth: r.from_month,
  }));
}
...
function dbToMonthly(rows) {
  ...
  rows.forEach(r => {
    m[`${r.ac_id}|${r.year}|${r.month}`] = {
      _dbId: r.id, heures: Number(r.heures) || 0, rotations: Number(r.rotations) || 0,
    };
  });
  return m;
}
...
export function useFleetData() {
  ...
  const addAircraft = useCallback(async (immat, type) => {
    const { data: row, error } = await supabase.from("aircraft").insert({ immat, type }).select().single();
    if (error) { console.error(error); return null; }
    const ac = { id: row.id, immat: row.immat, type: row.type };
    setData(d => ({ ...d, aircraft: [...d.aircraft, ac] }));
    return ac;
  }, []);

  const updateAircraft = useCallback(async (id, immat, type) => {
    await supabase.from("aircraft").update({ immat, type }).eq("id", id);
    setData(d => ({ ...d, aircraft: d.aircraft.map(a => a.id === id ? { ...a, immat, type } : a) }));
  }, []);
  ...
  const addRate = useCallback(async (acId, field, value, fromYear, fromMonth) => {
    const { data: row, error } = await supabase.from("rates")
      .insert({ ac_id: acId, field, value, from_year: fromYear, from_month: fromMonth })
      .select().single();
    if (error) { console.error(error); return; }
    const rate = { id: row.id, acId, field, value: Number(value), fromYear, fromMonth };
  ...
  const setMonthly = useCallback(async (acId, year, month, heures, rotations) => {
    const key = `${acId}|${year}|${month}`;
    const existing = data.monthly[key];

    if (existing && existing._dbId) {
      await supabase.from("monthly").update({ heures, rotations }).eq("id", existing._dbId);
      setData(d => ({
        ...d,
        monthly: { ...d.monthly, [key]: { ...d.monthly[key], heures, rotations } },
      }));
    } else {
      const { data: row, error } = await supabase.from("monthly")
        .upsert({ ac_id: acId, year, month, heures, rotations }, { onConflict: "ac_id,year,month" })
        .select().single();
      if (error) { console.error(error); return; }
      setData(d => ({
        ...d,
        monthly: { ...d.monthly, [key]: { _dbId: row.id, heures, rotations } },
      }));
    }
  }, [data.monthly]);
  ...
  const addOp = useCallback(async (op) => {
    const { data: row, error } = await supabase.from("ops")
      .insert({
        ac_id: op.acId, year: op.year, month: op.month,
        cost: op.cost, label: op.label, description: op.desc || "", type: op.type,
      })
      .select().single();
    if (error) { console.error(error); return; }
    const o = { ...op, id: row.id };

...
function dbToMonthly(rows) {
  ...
  rows.forEach(r => {
    m[`${r.ac_id}|${r.year}|${r.month}`] = {
      ...
      heuresDc: Number(r.heures_dc) || 0, rotationsDc: Number(r.rotations_dc) || 0,
    };
  });
  return m;
}
...
export function useFleetData() {
  ...
  const setMonthly = useCallback(async (acId, year, month, heures, rotations, heuresDc, rotationsDc) => {
    const key = `${acId}|${year}|${month}`;
    const existing = data.monthly[key];
    const dbRow = { heures, rotations, heures_dc: heuresDc || 0, rotations_dc: rotationsDc || 0 };
    const stateRow = { heures, rotations, heuresDc: heuresDc || 0, rotationsDc: rotationsDc || 0 };

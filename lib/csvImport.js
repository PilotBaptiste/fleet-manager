...
export function parseFlightCSV(csvText, filterYear) {
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    // Duration in minutes — parse Aerogest "58 min (00:58)", HH:MM, or plain number
    let dureeMin = 0;
    const dureeRaw = get(row, "dureeMin");
    if (dureeRaw) {
      // 1) "58 min" or "58 min (00:58)" → extract minute count
      const minMatch = dureeRaw.match(/(\d+)\s*min/i);
      if (minMatch) {
        dureeMin = parseInt(minMatch[1]);
      } else {
        // 2) HH:MM or HhMM → convert to minutes
        const hhmm = dureeRaw.match(/^(\d{1,3})[:hH](\d{1,2})$/);
        if (hhmm) {
          dureeMin = parseInt(hhmm[1]) * 60 + parseInt(hhmm[2]);
        } else {
          // 3) Plain number → treat as minutes
          const v = parseFloat(dureeRaw.replace(",", "."));
          if (!isNaN(v)) dureeMin = Math.round(v);
        }
      }
    }

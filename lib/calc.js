...
export function getActivity(data, acId, year, month) {
  return data.monthly[`${acId}|${year}|${month}`] || {
    ...
    litresCarburant: 0,
    volsDecouverte: 0, volsInitiation: 0, volsBia: 0,
    revenuDecouverte: 0, revenuInitiation: 0, revenuBia: 0,
  };
}
...
export function calcMonth(data, acId, y, m) {
  ...
  // Vols spéciaux : revenus saisis directement (tarifs multiples)
  const volsDecouverte = act.volsDecouverte || 0;
  const volsInitiation = act.volsInitiation || 0;
  const volsBia = act.volsBia || 0;
  const revenuDecouverte = act.revenuDecouverte || 0;
  const revenuInitiation = act.revenuInitiation || 0;
  ...
  return {
    ...
    volsDecouverte, volsInitiation, volsBia,
...
const AGG_KEYS = ["heures","rotations","hCdb","hDc","hDec","hInit","hBia","heuresPilote","volsDecouverte","volsInitiation","volsBia","revenu","revenuVol","revenuVolCdb","revenuVolDc","revenuPilote","revenuDecouverte","revenuInitiation","revenuBia","revenuRoulage","litresCarburant","fixe","variable","depenses","depensesNormales","resultat","resultatRecurrent","loan","opsC","opsCNormal","opsCExceptional","carburant"];

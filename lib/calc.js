...
export function breakEvenHours(data, acId, year, ms, overrideTarif, overrideForfait) {
  const lm = ms[ms.length - 1] || 0;
  const tarif = overrideTarif !== undefined ? overrideTarif : getRate(data.rates, acId, "tarifHeure", year, lm);
  const forfait = overrideForfait !== undefined ? overrideForfait : getRate(data.rates, acId, "forfaitRoulage", year, lm);
  ...
  const revenuePerH = tarif + rotPerH * forfait;

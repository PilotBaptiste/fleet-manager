...
export function calcMonth(data, acId, y, m) {
  ...
  const forfaitMin = (ac && ac.blockBlock) ? 0 : getGlobalRate(data.rates, "forfaitRoulage", y, m);
  ...
  const carburant = heures * getRate(data.rates, acId, "consoCarburant", y, m) * prixCarbu;
...
export function breakEvenHours(data, acId, year, ms, overrideTarif, overrideForfaitMin) {
  ...
  const acObj = (data.aircraft || []).find(a => a.id === acId);
  const tarif = overrideTarif !== undefined ? overrideTarif : getRate(data.rates, acId, "tarifHeure", year, lm);
  const isBlockBlock = acObj?.blockBlock;
  const forfaitMin = isBlockBlock ? 0 : (overrideForfaitMin !== undefined ? overrideForfaitMin : getGlobalRate(data.rates, "forfaitRoulage", year, lm));
  const forfaitEur = (forfaitMin / 60) * tarif;

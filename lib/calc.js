...
export const RATE_GROUPS = [
  { group: "Tarification", fields: [
    { key: "tarifHeure", label: "Tarif CdB — Commandant de bord (€/h)", step: "1" },
    { key: "tarifHeureDC", label: "Tarif DC — Double commande (€/h)", step: "1" },
...
export function getActivity(data, acId, year, month) {
  return data.monthly[`${acId}|${year}|${month}`] || { heures: 0, rotations: 0, heuresDc: 0, rotationsDc: 0 };
}
...
export function calcMonth(data, acId, y, m) {
  ...
  // CdB = Commandant de bord, DC = Double commande
  const hCdb = act.heures || 0, rotCdb = act.rotations || 0;
  const hDc = act.heuresDc || 0, rotDc = act.rotationsDc || 0;
  const heures = hCdb + hDc;
  const rotations = rotCdb + rotDc;

  const tarifCdb = getRate(data.rates, acId, "tarifHeure", y, m);
  const tarifDc = getRate(data.rates, acId, "tarifHeureDC", y, m) || tarifCdb;
  const forfaitMin = getRate(data.rates, acId, "forfaitRoulage", y, m);

  // Revenu vol = heures × tarif correspondant
  const revenuVolCdb = hCdb * tarifCdb;
  const revenuVolDc = hDc * tarifDc;
  const revenuVol = revenuVolCdb + revenuVolDc;

  // Roulage = minutes/60 × tarif correspondant × rotations
  const forfaitEurCdb = (forfaitMin / 60) * tarifCdb;
  const forfaitEurDc = (forfaitMin / 60) * tarifDc;
  const revenuRoulageCdb = rotCdb * forfaitEurCdb;
  const revenuRoulageDc = rotDc * forfaitEurDc;
  const revenuRoulage = revenuRoulageCdb + revenuRoulageDc;
  ...
  const carburant = heures * getRate(data.rates, acId, "consoCarburant", y, m) * getRate(data.rates, acId, "prixCarburant", y, m);
  const huile = heures * getRate(data.rates, acId, "consoHuile", y, m) * getRate(data.rates, acId, "prixHuile", y, m);
  ...
  return {
    heures, rotations, hCdb, hDc, rotCdb, rotDc,
    tarifCdb, tarifDc, forfaitMin, forfaitEurCdb, forfaitEurDc,
    revenu, revenuVol, revenuVolCdb, revenuVolDc, revenuRoulage, revenuRoulageCdb, revenuRoulageDc,
    fixe, variable, depenses, resultat, coutH, carburant, huile, mainVar, loan, opsC,
  };
}

const AGG_KEYS = ["heures","rotations","hCdb","hDc","rotCdb","rotDc","revenu","revenuVol","revenuVolCdb","revenuVolDc","revenuRoulage","revenuRoulageCdb","revenuRoulageDc","fixe","variable","depenses","resultat","loan","opsC","carburant"];
...
export function globAgg(data, y, ms) {
  let t = {}; AGG_KEYS.forEach(k => t[k] = 0);
  data.aircraft.forEach(ac => { const a = aggAC(data,ac.id,y,ms); AGG_KEYS.forEach(k => t[k] += a[k]); });
  return t;
}
...
export function calcMonthWithOverrides(data, acId, y, m, overrides) {
  ...
  const rateFields = ["tarifHeure","tarifHeureDC","forfaitRoulage","assurance","hangar","maintenanceFixe","redevance","navigabilite","diversFixe","prixCarburant","consoCarburant","prixHuile","consoHuile","maintenanceHoraire"];
...
export function breakEvenHours(data, acId, year, ms, overrideTarifCdb, overrideTarifDc, overrideForfaitMin) {
  const lm = ms[ms.length - 1] || 0;
  const tarifCdb = overrideTarifCdb !== undefined ? overrideTarifCdb : getRate(data.rates, acId, "tarifHeure", year, lm);
  const tarifDcBase = getRate(data.rates, acId, "tarifHeureDC", year, lm);
  const tarifDc = overrideTarifDc !== undefined ? overrideTarifDc : (tarifDcBase || tarifCdb);
  ...
  // Use actual CdB/DC mix ratio to compute weighted average revenue/hour
  const actual = aggAC(data, acId, year, ms);
  const totalH = actual.heures || 1;
  const ratioCdb = actual.hCdb / totalH;
  const ratioDc = actual.hDc / totalH;
  const avgTarif = ratioCdb * tarifCdb + ratioDc * tarifDc;
  const avgForfaitEur = (forfaitMin / 60) * avgTarif;
  const rotPerH = actual.heures > 0 ? actual.rotations / actual.heures : 1;
  const revenuePerH = avgTarif + rotPerH * avgForfaitEur;

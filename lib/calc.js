...
export function calcMonth(data, acId, y, m) {
  ...
  // Revenu vol = heures totales × tarif unique (DC gratuite = même tarif)
  const revenuVolCdb = hCdb * tarif;
  const revenuVolDc = hDc * tarif;
  const revenuVol = revenuVolCdb + revenuVolDc;

  // Roulage = minutes/60 × tarif × rotations
  const revenuRoulageCdb = rotCdb * forfaitEur;
  const revenuRoulageDc = rotDc * forfaitEur;

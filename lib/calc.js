...
export const RATE_GROUPS = [
  { group: "Tarification — par type de vol", fields: [
    ...
    { key: "tarifDecouverte", label: "Vol baptême (€/passager)", step: "1", help: "Tarif par passager. Revenu = nb passagers × tarif" },
...
export function getActivity(data, acId, year, month) {
  return data.monthly[`${acId}|${year}|${month}`] || {
    ...
    litresCarburant: 0,
    volsDec1: 0, volsDec2: 0, volsDec3: 0,
  };
}
...
export function calcMonth(data, acId, y, m) {
  ...
  // Vols baptême : revenu = nb passagers (1×n1 + 2×n2 + 3×n3) × tarif/pers
  const volsDec1 = act.volsDec1 || 0;
  const volsDec2 = act.volsDec2 || 0;
  const volsDec3 = act.volsDec3 || 0;
  const volsDecouverte = volsDec1 + volsDec2 + volsDec3;
  const passagersDecouverte = volsDec1 + 2*volsDec2 + 3*volsDec3;
  const revenuDecouverte = passagersDecouverte * tarifDec;
  ...
  return {
    ...
    volsDec1, volsDec2, volsDec3, volsDecouverte, passagersDecouverte,
...
const AGG_KEYS = ["heures","rotations","hCdb","hDc","hDec","hInit","hBia","heuresPilote","volsDec1","volsDec2","volsDec3","volsDecouverte","passagersDecouverte","revenu","revenuVol","revenuVolCdb","revenuVolDc","revenuPilote","revenuDecouverte","revenuInitiation","revenuBia","revenuRoulage","litresCarburant","fixe","variable","depenses","depensesNormales","resultat","resultatRecurrent","loan","opsC","opsCNormal","opsCExceptional","carburant"];

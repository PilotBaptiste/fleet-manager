...
export const GLOBAL_RATE_FIELDS = [
  { key: "forfaitRoulage", label: "Forfait roulage (min)", step: "1", help: "Minutes facturées au tarif horaire, appliqué à chaque vol" },
];
...
export function calcMonth(data, acId, y, m) {
  ...
  const forfaitMin = getGlobalRate(data.rates, "forfaitRoulage", y, m);
  ...
  const revenuRoulage = rotations * forfaitEur;
  ...
  return {
    heures, rotations, hCdb, hDc,
    tarif, forfaitMin, forfaitEur,
    revenu, revenuVol, revenuVolCdb, revenuVolDc, revenuRoulage,
    fixe, variable, depenses, resultat, coutH, carburant, huile, mainVar, loan, opsC,
  };
}

const AGG_KEYS = ["heures","rotations","hCdb","hDc","revenu","revenuVol","revenuVolCdb","revenuVolDc","revenuRoulage","fixe","variable","depenses","resultat","loan","opsC","carburant"];
...
export function breakEvenHours(data, acId, year, ms, overrideTarif, overrideForfaitMin) {
  ...
  const forfaitMin = overrideForfaitMin !== undefined ? overrideForfaitMin : getGlobalRate(data.rates, "forfaitRoulage", year, lm);
  const forfaitEur = (forfaitMin / 60) * tarif;

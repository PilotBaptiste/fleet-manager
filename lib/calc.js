...
export const RATE_GROUPS = [
  { group: "Tarification", fields: [
    ...
    { key: "tarifDec1pax", label: "Vol découverte 1 pax (€/vol)", step: "1" },
    { key: "tarifDec2pax", label: "Vol découverte 2 pax (€/vol)", step: "1" },
    { key: "tarifDec3pax", label: "Vol découverte 3 pax (€/vol)", step: "1" },
    { key: "tarifInitiation", label: "Vol d'initiation (€/vol)", step: "1" },
    { key: "tarifVoltigeDec", label: "Voltige découverte (€/vol)", step: "1" },
    { key: "tarifVintage", label: "Vol vintage (€/vol)", step: "1" },
  ]},
...
export function getActivity(data, acId, year, month) {
  return data.monthly[`${acId}|${year}|${month}`] || {
    ...
    litresCarburant: 0,
    volsDec1pax: 0, volsDec2pax: 0, volsDec3pax: 0,
    volsInitiation: 0, volsBia: 0,
    revenuBia: 0,
    heuresVoltigeDec: 0, volsVoltigeDec: 0,
    heuresVintage: 0, volsVintage: 0,
  };
}
...
export function calcMonth(data, acId, y, m) {
  ...
  const hVoltigeDec = act.heuresVoltigeDec || 0;
  const hVintage = act.heuresVintage || 0;
  const heuresPilote = hCdb + hDc;
  const heures = heuresPilote + hDec + hInit + hBia + hVoltigeDec + hVintage;
  ...
  // Vols découverte : tarif par catégorie pax (auto-calculé)
  const volsDec1pax = act.volsDec1pax || 0;
  const volsDec2pax = act.volsDec2pax || 0;
  const volsDec3pax = act.volsDec3pax || 0;
  const volsDecouverte = volsDec1pax + volsDec2pax + volsDec3pax;
  const tarifDec1 = getRate(data.rates, acId, "tarifDec1pax", y, m);
  const tarifDec2 = getRate(data.rates, acId, "tarifDec2pax", y, m);
  const tarifDec3 = getRate(data.rates, acId, "tarifDec3pax", y, m);
  const revenuDecouverte = volsDec1pax * tarifDec1 + volsDec2pax * tarifDec2 + volsDec3pax * tarifDec3;
  // Vol initiation : tarif unique (auto-calculé)
  const volsInitiation = act.volsInitiation || 0;
  const tarifInit = getRate(data.rates, acId, "tarifInitiation", y, m);
  const revenuInitiation = volsInitiation * tarifInit;
  // Vol BIA : saisie directe du revenu total
  const volsBia = act.volsBia || 0;
  const revenuBia = act.revenuBia || 0;
  // Voltige découverte : tarif par vol
  const volsVoltigeDec = act.volsVoltigeDec || 0;
  const tarifVoltigeDec = getRate(data.rates, acId, "tarifVoltigeDec", y, m);
  const revenuVoltigeDec = volsVoltigeDec * tarifVoltigeDec;
  // Vol vintage : tarif par vol
  const volsVintage = act.volsVintage || 0;
  const tarifVintage = getRate(data.rates, acId, "tarifVintage", y, m);
  const revenuVintage = volsVintage * tarifVintage;

  const revenuVol = revenuPilote + revenuDecouverte + revenuInitiation + revenuBia + revenuVoltigeDec + revenuVintage;
  ...
  return {
    heures, rotations, hCdb, hDc, hDec, hInit, hBia, hVoltigeDec, hVintage, heuresPilote,
    volsDec1pax, volsDec2pax, volsDec3pax, volsDecouverte, volsInitiation, volsBia, volsVoltigeDec, volsVintage,
    tarif, tarifDec1, tarifDec2, tarifDec3, tarifInit, tarifVoltigeDec, tarifVintage,
    revenu, revenuVol, revenuVolCdb, revenuVolDc, revenuPilote,
    revenuDecouverte, revenuInitiation, revenuBia, revenuVoltigeDec, revenuVintage,
...
const AGG_KEYS = ["heures","rotations","hCdb","hDc","hDec","hInit","hBia","hVoltigeDec","hVintage","heuresPilote","volsDec1pax","volsDec2pax","volsDec3pax","volsDecouverte","volsInitiation","volsBia","volsVoltigeDec","volsVintage","revenu","revenuVol","revenuVolCdb","revenuVolDc","revenuPilote","revenuDecouverte","revenuInitiation","revenuBia","revenuVoltigeDec","revenuVintage","litresCarburant","variable","depenses","depensesNormales","resultat","resultatRecurrent","loan","opsC","opsCNormal","opsCExceptional","carburant"];
...
export function calcMonthWithOverrides(data, acId, y, m, overrides) {
  ...
  const rateFields = ["tarifHeure","tarifDec1pax","tarifDec2pax","tarifDec3pax","tarifInitiation","tarifVoltigeDec","tarifVintage","consoCarburant","consoHuile"];
